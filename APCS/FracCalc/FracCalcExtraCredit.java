public class FracCalcExtraCredit {
	...
	public static void main(String[] args) {
		System.out.println(produceAnswer("1/4 + 2/4 + 3/4 + 5/4 + 1/4 + 2/4"));
	}
					...
					if (iNum > iC) {
						...
						if (iC % iNum != 0 || iNum != 0) {
							...
							if (iNum == 0) {
								...
								if (splitUp.length > 3 && repCount <= (0.5 * splitUp.length - 1.5)) {

									for (int r = 3; r <= splitUp.length - 1; r++) {
										finalAnswer = finalAnswer + " " + splitUp[r];
									}

									System.out.println(produceAnswer(finalAnswer));
									repCount++;
								}
							} else {
								String stringWhole = Integer.toString(iWhole);
